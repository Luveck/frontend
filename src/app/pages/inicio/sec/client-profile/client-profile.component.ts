...
export class ClientProfileComponent implements OnInit {
  ...
  private async loadInformations() {
    this.isLoadingResults = true;
    this.userData = await this.authService.getUserById(
      this.sessionService.getUserData().UserId
    );
  ...
  selectRole(nameRole: string) {
    const tempRole = this._usersServ.localRoles.filter(
  ...
  save(chageState?: boolean) {
    ...
    tempData.idRole = id;
    tempData.role = name;
    const peticion = this._usersServ.UpdateUsuario(
      tempData,
      chageState != undefined ? chageState : this.userData.state
    );
    peticion?.subscribe(
      () => {
