  ...
  private async Register() {
    try {
      ...
      if  (error.error && error.error.includes('is already taken')) {
        this.sharedService.notify(`El DNI ${this.registerForm.value.dni} ya esta siendo usado.`, 'error');
      } else {
        this.sharedService.notify(`Se presento un error, por favor contactar a soporte.`, 'error');
      }
    } finally {
      this.dataServ.progress = false;
    }
